...
// 断线重连
@Component
public class HBiTexConnectRetryer implements InitializingBean {

    private static final Logger logger = LoggerFactory.getLogger(HBiTexConnectRetryer.class); 
	private static final ThreadPoolFactoryLow threadPool = ThreadPoolFactoryLow.create("traditex-inbox-断线重连线程池", "RECONNECT");
	...
	public void afterPropertiesSet() throws Exception {
        threadPool.submit(this::reconnectLoop);
	}

    // 将连接对象放入延迟队列等待重连
    public void add(TradiTexConnection<?> connection) {
        reconnectDealyQueue.put(new DelayedItem<>(connection, RECONNECT_TIME_SEC, TimeUnit.SECONDS));
    }
